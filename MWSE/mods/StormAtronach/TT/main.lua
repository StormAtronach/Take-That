...
mwse.memory.writeBytes {
	address = 0x541530,
	bytes = {0x8B,0x15,0xDC,0x67,0x7C,0x00,0xD9,0x42,0x2C,0x8B,0x41,0x3C,0xD8,0x88,0xDC,0x04,0x00,0x00,0x8B,0x51,0x38,0x8D,0x82,0xCC,0x00,0x00,0x00,0xD8,0x40,0x08,0xD9,0x58,0x08,0xC7,0x41,0x10,0x00,0x00,0x80,0xBF,0xC6,0x40,0x11,0x03,0xD9,0x41,0x2C,0xD8,0x1D,0x68,0x64,0x74,0x00,0xDF,0xE0,0xF6,0xC4,0x40,0x75,0x0B,0x8B,0x41,0x2C,0x89,},
}
mwse.memory.writeBytes {
	address = 0x5414E0,
...
mwse.memory.writeBytes {
	address = 0x54147A,
	bytes = { 0x8B, 0x90, 0x88, 0x03, 0, 0, 0x0A, 0x90, 0x28, 0x02, 0, 0, 0x85, 0xD2, 0x75, 0x04, 0x84, 0xDB, 0x75, 0x3D },
}
...
local function parrySlowDown(e)
	if not config.parrySlowDown then
		return
	end
	...
	elseif tes3.mobilePlayer.animationController:calculateAttackSwing() > 0.9 then
		...
		if mainSlowmoState == nil then
			mainSlowmoState = { phase = "in", phaseStart = os.clock(), targetScalar = config.parry_slowmo_scalar }
...
local function getWeaponNode(ref)
	...
	return ref.sceneNode:getObjectByName("Weapon") or ref.sceneNode:getObjectByName("Bip01 R Hand")
end
...
local function drawDebugLine(name, origin, destination, r, g, b)
	...
	if line == nil then
		line = tes3.loadMesh("mwse\\widgets.nif", false):getObjectByName("axisLines"):getObjectByName("z"):clone() --[[@as niTriShape]]
		line.name = name
...
local function spawnTrailLine(origin, destination, r, g, b, label, duration)
	local now = os.clock()
	if (trailLineLastTime[label] or 0) + 0.025 > now then
		return
	end
...
onSimulate_collisionParry = function()
	...
	local refs = {}
	for ref in pairs(activeWeaponTrackers) do
		refs[#refs + 1] = ref
	end
	if #refs < 2 then
		return
		...
		for j = i + 1, #refs do
			...
			if dist < config.parry_collision_threshold then
				...
				if config.enemy_parry_active then
					if refA ~= tes3.player then
						common.parryingActors[refA] = true
					end
					if refB ~= tes3.player then
						common.parryingActors[refB] = true
					end
				end
...
local function resetState()
	...
	tes3.playAnimation({ reference = animReference, group = 0 })
...
local function attackHitCallback(e)
	...
	log:debug("attackHit parry check: attacker=%s target=%s parryFlag=%s", ID, e.targetReference.id,
	          tostring(common.parryingActors[e.targetReference]))
...
local function onAttack(e)
	...
	if not playerIsThatYou and e.targetReference and
	(e.targetReference == tes3.player or e.targetReference == tes3.player1stPerson) then
	...
	if config.force_counter_enabled and not playerIsThatYou then
		if e.targetReference == tes3.player or e.targetReference == tes3.player1stPerson then
			...
			local now = os.clock()
			if mob.readiedWeapon and mob.actionData.attackSwing > 0 and now - (forceCounterLastTime or 0) >=
			config.force_counter_cooldown then
...
local function initialized()
	...
	-- ── NPC reactions + VFX ───────────────────────────────────────────────────
	npcDodge.init()
	local sparks = tes3.createObject {
		objectType = tes3.objectType.static,
		id = "AXE_sa_VFX_WSparks",
		mesh = "e\\spark.nif",
	}
...
event.register(tes3.event.loaded, function() deactivate() end)
