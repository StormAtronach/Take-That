...
local function spellBatting()
	...
	if closestRef then
		incomingProjectiles[closestRef] = nil
	end
	local spellName = closestMob.spellInstance and closestMob.spellInstance.source.name or "?"
	log:debug("Player batted '%s' (fired by %s) at range %.0f", spellName,
	          closestMob.firingReference and closestMob.firingReference.id or "?", closestDist)
end
...
local function npcBatCallback(mobHandle, targetHandle, label)
	return function()
		...
		if not targetRef.mobile.canAct then
			return
		end
...
local function scheduleNpcBat(mob, direction, label)
	...
	local hit = tes3.rayTest({ position = mob.position, direction = direction, maxDistance = 5000,
                            ignore = { tes3.player } })
	if not (hit and hit.reference and hit.reference.mobile and hit.reference ~= tes3.player) then
		return
	end
	local targetRef = hit.reference
	local distance = hit.intersection:distance(mob.position)
	local eta = distance / speed
	log:debug("%s: %s speed=%.0f dist=%.0f eta=%.2fs", label, targetRef.id, speed, distance, eta)
	timer.start({
...
function public.onMobileActivated(e)
	...
	log:trace("onMobileActivated: tracking spell '%s' fired by %s", mob.spellInstance.source.name,
...
function public.onAttack(e)
	if not config.npc_spellbat_enabled then
		return
	end
	local attacker = e.mobile
	if not attacker or attacker == tes3.mobilePlayer then
		return
	end
	if math.random(100) > config.npc_spellbat_chance then
		return
	...
	local spellName = closestMob.spellInstance and closestMob.spellInstance.source.name or "?"
	log:debug("%s naturally batted '%s' at range %.0f", e.reference and e.reference.id or "?", spellName, closestDist)
end
...
handleReflection = function(mob, vel, reflector)
	if not (mob and mob.reference) then
		return
	end
	-- Process each projectile only once.
	if processedReflections[mob.reference] then
		return
	end
	...
	if not tcomActive then
		local vfx = tes3.createVisualEffect({ reference = mob.reference, object = "sa_trail", lifespan = 1 })
		if vfx then
