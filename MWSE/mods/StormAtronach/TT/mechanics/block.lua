...
function block.onSimulated()
	if not sets.shield1st.controllers then
		return
	end
	...
	if isHeld and tes3.menuMode() then
		isHeld = false
	end

	if isHeld ~= prevIsHeld then
		log:debug("Block key: %s (attacking=%s menuMode=%s)", isHeld and "HELD" or "released",
	...
	if block.active ~= prevActive then
		log:debug("Block %s (blockTime=%.3f 3rdPerson=%s hasShield=%s)", block.active and "RAISED" or "LOWERED", blockTime,
...
function block.onDamage(e)
	...
	if doYouHaveShield and (not doYouHaveWeapon or oneHanded) then
		...
		if shield and shield.itemData and shield.itemData.condition then
			shield.itemData.condition =
			math.max(0, math.ceil(shield.itemData.condition - originalDamage * reductionFactor / 100))
		end
	...
	if doYouHaveWeapon then
		local mySkillCheck = common.weaponSkillCheck({
			thisMobileActor = tes3.mobilePlayer,
			weapon = weapon and weapon.object.type,
		})
		...
		if config.block_skill_bonus_active then
			reductionFactor = math.clamp(config.block_shield_base_pc + config.block_weapon_skill_mult * mySkill +
			                             config.block_weapon_blockSkill_bonus * blockSkill, 0, 100)
		else
