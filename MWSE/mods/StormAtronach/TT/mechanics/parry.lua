...
local function onSpellCasted_blockEnchant(e)
	if e.caster ~= blockedEnchRef then
		return
	end
...
local parryOutcomes = {
	...
	[1] = {
		attacker = {
			...
			slowType = 1,
			slowDuration = 1,
			fatigueDrainKey = "parry_fatigue_drain_1",
		},
		defender = { hitStun = false, knockDown = false },
	},
	[2] = {
		attacker = {
			...
			slowType = 2,
			slowDuration = 1,
			fatigueDrainKey = "parry_fatigue_drain_2",
		},
		defender = { hitStun = false, knockDown = false },
	},
	[3] = {
		attacker = {
			...
			slowType = 3,
			slowDuration = 1,
			fatigueDrainKey = "parry_fatigue_drain_3",
		},
		defender = { hitStun = false, knockDown = false },
	},
}
...
local function damageWeapon(actor, damageAmount)
	...
	if not itemData then
		...
		itemData = tes3.addItemData({ to = actor.reference, item = weapon.object })
	end
...
function parry.attackHitCallback(e)
	...
	log:debug("Parry opposed check: defender(%d) - attacker(%d) = %d", defenderSkillLevel, attackerSkillLevel, opposedCheck)
	...
	attackerGuardTimers[aKey] = timer.start({
		duration = ATTACKER_GUARD,
		callback = function()
		...
		effects = {
			{
				...
				min = config.parry_light_magnitude,
				max = config.parry_light_magnitude,
				duration = config.parry_light_duration,
			},
		},
		name = "Parried!",
	})
