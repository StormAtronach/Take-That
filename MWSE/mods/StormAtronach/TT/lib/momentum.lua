...
function this.activate(mobile)
	...
	if not mobile.readiedWeapon then
		return
	end
...
function this.updateWeightScalar(ref)
	...
	local strengthMult = math.clamp(attribute / config.referenceStrength, config.strengthFloor, config.strengthCeiling)
...
function this.startRecovery(ref, swingCharge, weaponWeight)
	...
	local duration = math.lerp(config.recoveryMinDuration, config.recoveryMaxDuration, swingCharge * normalizedWeight)
	s.inRecovery = true
