...
function this.captureVanilla()
	...
	log:debug("Vanilla GMSTs captured - ReturnBase=%.2f ReturnMult=%.2f AttackBase=%.1f", vanilla.fatigueReturnBase,
	          vanilla.fatigueReturnMult, vanilla.fatigueAttackBase)
end
...
function this.apply()
	...
	log:debug("GMSTs applied - ReturnBase=%.2f ReturnMult=%.2f AttackBase=%.1f", config.fatigueReturnBase,
	          config.fatigueReturnMult, config.fatigueAttackBase)
end
...
function this.restoreVanilla()
	...
	log:debug("Vanilla GMSTs restored - ReturnBase=%.2f ReturnMult=%.2f AttackBase=%.1f", vanilla.fatigueReturnBase,
	          vanilla.fatigueReturnMult, vanilla.fatigueAttackBase)
end
...
function this.restoreDefaults()
	...
	log:debug("GMST defaults written to config - ReturnBase=%.2f ReturnMult=%.2f AttackBase=%.1f", d.fatigueReturnBase,
	d.fatigueReturnMult, d.fatigueAttackBase)
end
